<?php
  ...
  if (!isset($_SESSION['username'])) {
    ...
    {
        $images = getImagesVotedByUser(getUserId($_SESSION['username']));

        if(!empty($images))
        {
            foreach ($images as $image) {
                if($image['vot'] == 1) decrementaLike($image['fotografiaNom']);
                else decrementaDislike($image['fotografiaNom']);
                actualitzaPuntuacio($image['fotografiaNom']);
            }
        }

        eliminaUsuari(getUserId($_SESSION['username']));
                ...
                <div class="card border-0">
                    <div class="card-body p-0">
                      <div class="m-3 p-3 bg-white d-flex justify-content-between border-bottom">
                          <div class="col-6">
                              <h4 class="pb-2 mb-0 fw-bold">Eliminar compte</h4>
                          </div>
                          <div id="settingsIcons" class="justify-content-between col-6">
                                ...
                                <div class="col-12 div-wrapper d-flex flex-column justify-content-center align-items-center">
                                    ...
                                    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="mt-2 mb-5 w-75">
                                        ...
                                        <div class="mb-4 text-center">
                                          <h6 class="pb-2 mb-0 fw-bold"><i class="fas fa-exclamation-triangle me-2"></i>Eliminar el teu compte implica la pèrdua de totes les teves imatges pujades a Imaginest</h6>
                                        </div>
                                        <button type="submit" class="btn btn-danger w-100 fw-bold" name="deleteAccount">Eliminar</button>
                                      </form>
                                  </div>
                              </div>
                          </div>
