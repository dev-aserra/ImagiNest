<?php
  ...
  if (!isset($_SESSION['username'])) {
    ...
    {
      if(!empty($_POST['hashtag']) && isset($_POST['hashtag']))
      {
        $images = array();
        $hashtag = filter_input(INPUT_POST, 'hashtag');
        $arrayOfImages = getImagesByHashtag($hashtag);
        foreach($arrayOfImages as $image)
        {
          array_push($images,getImageByName($image));
        }
        if(empty($images)) $noPhotos = true;
      }
      else $images = getImageArray();
    }
    else $images = getImageArray();
  }
                ...
                <div class="card border-0">
                    <div class="card-body p-0">
                      <div class="m-3 p-3 bg-white rounded">
                        <h4 class="border-bottom pb-2 mb-0 fw-bold">Explora</h4>
                        <form id="search" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="mt-3">
                          <input maxlength="100" class="border rounded ms-2 form-control" placeholder="Cerca per hashtag..." type="text" name="hashtag" id="hashtag">
                      ...
                      <div class="images text-center pb-4">
                        <?php if(empty($images)):?>
                          <?php if(!isset($noPhotos)):?>
                            <h5>Encara no s'ha penjat cap imatge a la plataforma!</h5>
                          <?php else:?>
                            <h5>Encara no s'ha penjat cap imatge amb aquest hashtag!</h5>
                          <?php endif; ?>
                        <?php else:?>
